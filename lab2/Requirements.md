...
<, >, V, A - данными символами обозначается голова змейки, её направление.


$ - "яблочко" (специальный символ, наступив на который, "Змейка вырастет в размерах")
...
|:---|:---|
| Up | Поворот змейки вверх или продолжение движения вверх |
| Left | Поворот змейки налево или продолжение движения влево |
| Right | Поворот змейки направо или продолжение движения вправо |
| Down | Поворот змейки вниз или продолжение движения вниз |
| esc | Game over |
...
**Требования.** Очки должны начисляться в зависимости не только от сьеденного "яблочка", но и от частоты их поедания (выбиается разработчиком).

<a name="score_save"/>

#### 3.1.1.3 Сохранение очков.
**Описание.** Рекордное количество очков должно сохраняться, при последующих игровых сеансах.  
...
Данное приложение является полностью безопасным, т.к. оно не собирает какую-либо информацию о пользователе и не подразумевает доступа в Интернет.
